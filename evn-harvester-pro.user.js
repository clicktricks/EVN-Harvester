...
(function() {
    ...
    function exportHTML(station, dateObj, data) {
        ...
        triggerDownload(blob, `Report_${station}.html`);
    }

    function exportCSV(station, data) {
        // CSV-Header (Semicolon ist besser für deutsches Excel)
        let csv = "Zeit;Zug;Ziel;EVNs\n";
        data.forEach(item => {
            csv += `${item.time};${item.train};${item.dest};"${item.evns.join(", ")}"\n`;
        });

        const blob = new Blob(["\ufeff" + csv], { type: 'text/csv;charset=utf-8;' });
        triggerDownload(blob, `Liste_${station}.csv`);
    }
